...
function onData(data) {
	...
	if (this.flow) {
		if (data.slice(data.length - 5).toString() === '\r\n.\r\n') {
			this.flow.chunks.push(data.slice(data.length - 5));
			this.flow.length += data.length - 5;
					...
					if (this._command.callback) {
						if (this.mailparser) {
							...
							mp.on('end', function(mail) {
								this._command.callback.call(this, err, mail);
								this._command = {cmd: state.NOOP};
								this._runCommand();
							}.bind(this));
							mp.write(Buffer.concat(this.flow.chunks, this.flow.length));
							mp.end();
							delete this.flow;
						} else {
